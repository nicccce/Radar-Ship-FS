...
版本：`next-stage-roadmap-v4-post-3a-7-blocked`
...
任务 4C 已完成，K=32 锁定复核仍为 NO-GO：收益为 +0.1009 pp，3/2/0 正/平/负，两项效应门槛均未达到。当前静态 RL 分支停止，4D、5A、5B 和该分支的任务 6 均不准入。4C 没有训练 RL，不能把该停止决定解释为已证明 PPO 无效。任务 3A 已完成但未恢复真实场景/环境谱系；任务 7 已完成前提检查，因数据缺失而 BLOCKED / NO-GO。当前没有准入的训练任务；最新证据见 `03a-data-lineage-audit.md` 与 `07-environment-conditioned-feasibility.md`，4C 的历史解读见 `04c-review-and-next-steps.md`。
...
### 3.8 任务 3A 已完成，任务 7 因数据前提阻断

3A 未恢复可核验的 `base_scene_id`、增强父子关系或与最终行对应的海况、擦地角、SCR；现行 radar loader 的 `groups=None`。因此真实场景分组验证不可执行，既不能证明存在同源泄漏，也不能排除它。

任务 7 于 2026-09-21 完成准入检查，结论为 **BLOCKED / NO-GO（数据前提失败）**。未运行全局/分环境静态、监督条件化、contextual bandit 或 PPO；J 请求、唯一评分子集、分类器拟合和 source-test 内容读取均为 0。这不能解释为上述方法性能失败。

任务 7 确认的是 3A 已发现的数据阻断，没有增加算法性能证据。没有新的原始数据或生成记录时，不再重复同类审计。旧的效应门槛及任务 7 的样本覆盖门槛是本计划的资源投入和实验设计规则，不是 RL 不可行的证明，也不是所有新研究必须沿用的普遍定律；不得为通过旧任务而事后修改门槛。

输入证据：`03a-data-lineage-audit.md`、`07-environment-conditioned-feasibility.md`、`07-environment-conditioned-feasibility-protocol.md`。
...
| 3A | 审计已完成；谱系未恢复 | 缺少新材料时不重复审计 |
| 7 | 前提检查已完成；BLOCKED / NO-GO | 补齐真实 scene/parent/env 及选择前可用性后才可重新评估 |

**当前没有可以继续执行的训练 prompt。** 暂停在同一数据上增加 PPO 预算、扫描 K/reward 或重复准入审计。4A 的支持域修复、4C-0 的兼容性修复及已完成审计仍是有效交付，不能把所有任务统称为性能失败。

2026-09-21 后续更新：用户要求规划新的探索。执行入口为 `08-next-exploration-planning-prompt.md`，本轮先比较搜索效率、非线性交互与原问题的显式目标路线，选定一个小实验；不自动重启旧 4D/5A/5B。旧门槛不应被扩大成对所有新问题的禁止。当前数据可用于明确标注的机制探索；真实场景/环境泛化仍受数据前提限制。

若要恢复真实场景或环境路线，需要取回原始场景或实际生成链，补齐最终行到场景、增强父样本和环境的可验证映射；若旧映射不可恢复但原始场景仍在，则发布保留这些字段的新版本，先按场景划分再在训练分区内增强。若这些材料无法获得，该路线继续阻断；仍可规划现有数据上的探索性机制实验，或有清楚适用边界的显式目标/可控实例研究。不要从当前特征相似度推造场景或环境。

补齐数据不会自动重开旧 4C。任何新问题都需要明确的目标、合适的验证设计和新的协议；目前没有正当依据将“RL 优于 DFS/贪心”写成研究结论。

若研究目标改为“在可接受性能损失下减少特征计算成本”，应先证明各特征确有可减少的计算成本，并在新的评价数据开放前确定可接受损失。当前 32 维结果只提供提出该假设的理由，不能把失败的精度增益实验改名为压缩成功。

若希望继续研究 RL，应明确相对简单算法的优势来源：例如从多个优化实例学习可复用的搜索策略，或在实际特征获取过程中依据已观测值选择下一项并学习停止。前者需要验证跨实例泛化及训练成本摊销，后者需要可解释的获取成本和部署需求。任务 7 的环境条件化路线仍受数据前提阻断；不能把这一阻断外推到所有上述问题。

## 6. 历史任务定义与重启条件

**本节保留旧设计用于追溯。3A、4C-0、4C 及任务 7 前提检查均已完成；4D/5A/5B/6 因 4C NO-GO 而关闭，任务 7 性能实验因数据前提失败而阻断。不能把旧的条件说明当作当前执行许可。**

### 任务 3A：v16n_2x_noise 数据谱系与场景分组审计

该审计已完成，报告为 `03a-data-lineage-audit.md`，结论是谱系未恢复、真实 group-aware 验证不可执行。以下保留原任务要求；没有新材料时不重复执行，也不重新挑选划分使旧 4C 过关。
...
当前前提检查已完成，报告为 `07-environment-conditioned-feasibility.md`，结论为数据前提 BLOCKED / NO-GO。以下为历史设计；补齐真实数据前不执行性能实验。
